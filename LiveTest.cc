...
//g++ LiveTest.cc -o LiveTest -lavcodec -lavutil -levent -lSDL2 --sanitize=address
...
class DecodeContext{
    ...
    void StartPullStream(std::string url){
        ...
        */
        ...
        auto ev = event_new(base, -1, EV_PERSIST, [](evutil_socket_t ,short ,void *arg){
            DecodeContext *ctx = (DecodeContext *)arg;
            printf("test: step: %d\n", ctx->decodeIndex - ctx->playIndex);
            if (ctx->decodeIndex - ctx->playIndex >= 40 && ctx->decodeIndex - ctx->playIndex <= 80)
                return;
            auto bufev = evhttp_connection_get_bufferevent(ctx->pullconn);
            if (ctx->decodeIndex - ctx->playIndex > 80){
                if (bufferevent_get_enabled(bufev) & EV_READ){
                    bufferevent_disable(bufev, EV_READ);
                   //printf"test: disable read\n");
                }
            }
            if (ctx->decodeIndex - ctx->playIndex < 40){
                if (!(bufferevent_get_enabled(bufev) & EV_READ)){
    ...
    int &width;
    int &height;
    std::atomic<size_t> playIndex = {0};
    std::atomic<size_t> decodeIndex = {0};
    evhttp_connection *pullconn = nullptr;
    evbuffer *input;
    std::mutex m;
   ...
   //printf"SDL get width: %d, height: %d\n", width, height);
	...
	while (true) {
		...
		if(event.type == LOADPIC_EVENT){
            printf("decodedFrames_size: %d, localindex: %d\n", decodedFrames.size(), index);
